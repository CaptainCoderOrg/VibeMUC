...
namespace VibeMUC.Map
{
    public class DungeonMapGenerator
    {
        ...
        {
            return GenerateAsciiMap(false);
        }

        public void PrintColoredAscii()
        {
            GenerateAsciiMap(true);
        }

        private string GenerateAsciiMap(bool useColor)
        {
            // Create a character array for the ASCII representation
            // Each cell takes 3x3 characters to represent walls and doors
            char[,] asciiMap = new char[_height * 3, _width * 3];

            // Initialize with empty space
            for (int y = 0; y < asciiMap.GetLength(0); y++)
            {
                for (int x = 0; x < asciiMap.GetLength(1); x++)
                {
                    asciiMap[y, x] = ' ';
                }
            }
            ...
            for (int y = 0; y < _height; y++)
            {
                for (int x = 0; x < _width; x++)
                {
                    ...
                    if (!cell.IsEmpty)
                    {
                        ...
                        // Fill center with floor marker
                        asciiMap[baseY + 1, baseX + 1] = '.';

                        // Add walls and doors
                        if (cell.HasNorthWall)
                            asciiMap[baseY, baseX + 1] = cell.HasNorthDoor ? '+' : '─';
                        if (cell.HasSouthWall)
                            asciiMap[baseY + 2, baseX + 1] = cell.HasSouthDoor ? '+' : '─';
                        if (cell.HasEastWall)
                            asciiMap[baseY + 1, baseX + 2] = cell.HasEastDoor ? '+' : '│';
                        if (cell.HasWestWall)
                            asciiMap[baseY + 1, baseX] = cell.HasWestDoor ? '+' : '│';

                        // Add corners if needed
                        if (cell.HasNorthWall || cell.HasEastWall)
                            asciiMap[baseY, baseX + 2] = '┐';
                        if (cell.HasNorthWall || cell.HasWestWall)
                            asciiMap[baseY, baseX] = '┌';
                        if (cell.HasSouthWall || cell.HasEastWall)
                            asciiMap[baseY + 2, baseX + 2] = '┘';
                        if (cell.HasSouthWall || cell.HasWestWall)
                            asciiMap[baseY + 2, baseX] = '└';
                    }
                }
            }

            // Convert to string or print with color
            ...
            if (useColor)
            {
                ...
                Console.WriteLine($"Map Size: {_width}x{_height}, Rooms: {_rooms.Count}");
            ...
            {
                result.AppendLine($"Map Size: {_width}x{_height}, Rooms: {_rooms.Count}");
                result.AppendLine("Legend: [.] Floor, [─│] Walls, [+] Doors, [┌┐└┘] Corners");
                result.AppendLine();
            }
            ...
            for (int y = 0; y < asciiMap.GetLength(0); y++)
            {
                for (int x = 0; x < asciiMap.GetLength(1); x++)
                {
                    ...
                    if (useColor)
                    {
                        ...
                        {
                            '.' => ConsoleColor.Green,    // Floor
                            '+' => ConsoleColor.Yellow,   // Doors
                            '─' or '│' or '┌' or '┐' or '└' or '┘' => ConsoleColor.White, // Walls and corners
                            _ => ConsoleColor.DarkGray    // Empty space
                        };
                        Console.Write(c);
                    }
                    else
            ...
            // Reset console color if we used colors
            if (useColor)
            {
                Console.ResetColor();
                return string.Empty;
        ...
        private DungeonMapData CreateMapData()
        {
            ...
            // Add ASCII representation to metadata
            map.Metadata["ascii_map"] = ToAscii();
